...
自然 fallthrough 必须沿有序语句 region 的尾路径完成，不能只凭 ancestor containment
认定。例如内层分支跳过外层尚未执行的 tail 时，该边必须保留显式 transfer。
`RegionNavigation` 为非尾 sequence/island child 冻结完成性屏障前缀，使查询保持 O(1)；
已冻结的 condition DAG 不是顺序语句列表，不参与该屏障。island 还须核对来源是否为实际
layout 尾部。sequence 按 CFG 顺序重排后必须重新冻结依赖顺序的完成性事实，不能使用
重排前的缓存决定跳转消除。
...
- 短路出口的单臂归一化必须证明另一臂只有该本轮正常 dominance frontier，排除函数
  终止出口、两端共同 loop owner 的退出与回到支配当前臂的循环头，不能仅凭 frontier
  membership 推断 continuation。例如 `(a or b) and c`
  的部分条件可能同时抵达 else 入口与两臂的共享尾部；此时保留共同 soft merge 的
  `IfElse`，不能把 else 当 continuation 后将真正尾部吞进 then 的支配子树。
